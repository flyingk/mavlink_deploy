...
#define MAVLINK_MSG_ID_VEHICLE_STATE_MIN_LEN 60
#define MAVLINK_MSG_ID_198_LEN 60
#define MAVLINK_MSG_ID_198_MIN_LEN 60
                               ...
                               uint32_t time_boot_ms, int16_t N, int16_t E, int16_t D, float u, float v, float w, int16_t p, int16_t q, int16_t r, float Roll, float Pitch, float Yaw, float W_spd, float W_dir, float Vair, float AoA, float CW)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_VEHICLE_STATE_MIN_LEN, MAVLINK_MSG_ID_VEHICLE_STATE_LEN, MAVLINK_MSG_ID_VEHICLE_STATE_CRC);
}
                                   ...
                                   uint32_t time_boot_ms,int16_t N,int16_t E,int16_t D,float u,float v,float w,int16_t p,int16_t q,int16_t r,float Roll,float Pitch,float Yaw,float W_spd,float W_dir,float Vair,float AoA,float CW)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_VEHICLE_STATE;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_VEHICLE_STATE_MIN_LEN, MAVLINK_MSG_ID_VEHICLE_STATE_LEN, MAVLINK_MSG_ID_VEHICLE_STATE_CRC);
}
...
static inline void mavlink_msg_vehicle_state_send(mavlink_channel_t chan, uint32_t time_boot_ms, int16_t N, int16_t E, int16_t D, float u, float v, float w, int16_t p, int16_t q, int16_t r, float Roll, float Pitch, float Yaw, float W_spd, float W_dir, float Vair, float AoA, float CW)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_VEHICLE_STATE, buf, MAVLINK_MSG_ID_VEHICLE_STATE_MIN_LEN, MAVLINK_MSG_ID_VEHICLE_STATE_LEN, MAVLINK_MSG_ID_VEHICLE_STATE_CRC);
#else
    mavlink_vehicle_state_t packet;
    packet.time_boot_ms = time_boot_ms;
    packet.u = u;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_VEHICLE_STATE, (const char *)&packet, MAVLINK_MSG_ID_VEHICLE_STATE_MIN_LEN, MAVLINK_MSG_ID_VEHICLE_STATE_LEN, MAVLINK_MSG_ID_VEHICLE_STATE_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_vehicle_state_send_struct(mavlink_channel_t chan, const mavlink_vehicle_state_t* vehicle_state)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_vehicle_state_send(chan, vehicle_state->time_boot_ms, vehicle_state->N, vehicle_state->E, vehicle_state->D, vehicle_state->u, vehicle_state->v, vehicle_state->w, vehicle_state->p, vehicle_state->q, vehicle_state->r, vehicle_state->Roll, vehicle_state->Pitch, vehicle_state->Yaw, vehicle_state->W_spd, vehicle_state->W_dir, vehicle_state->Vair, vehicle_state->AoA, vehicle_state->CW);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_VEHICLE_STATE, (const char *)vehicle_state, MAVLINK_MSG_ID_VEHICLE_STATE_MIN_LEN, MAVLINK_MSG_ID_VEHICLE_STATE_LEN, MAVLINK_MSG_ID_VEHICLE_STATE_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_VEHICLE_STATE_LEN? msg->len : MAVLINK_MSG_ID_VEHICLE_STATE_LEN;
        memset(vehicle_state, 0, MAVLINK_MSG_ID_VEHICLE_STATE_LEN);
    memcpy(vehicle_state, _MAV_PAYLOAD(msg), len);
#endif
}
