...
#define MAVLINK_MSG_ID_PID_TUNE_MIN_LEN 28
                               ...
                               uint32_t time_boot_ms, float Cmd_In, float Sensor_Fbk, float Cmd_Out, float Kp, float Ki, float Kd)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_PID_TUNE_MIN_LEN, MAVLINK_MSG_ID_PID_TUNE_LEN, MAVLINK_MSG_ID_PID_TUNE_CRC);
}
                                   ...
                                   uint32_t time_boot_ms,float Cmd_In,float Sensor_Fbk,float Cmd_Out,float Kp,float Ki,float Kd)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_PID_TUNE;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_PID_TUNE_MIN_LEN, MAVLINK_MSG_ID_PID_TUNE_LEN, MAVLINK_MSG_ID_PID_TUNE_CRC);
}
...
static inline void mavlink_msg_pid_tune_send(mavlink_channel_t chan, uint32_t time_boot_ms, float Cmd_In, float Sensor_Fbk, float Cmd_Out, float Kp, float Ki, float Kd)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PID_TUNE, buf, MAVLINK_MSG_ID_PID_TUNE_MIN_LEN, MAVLINK_MSG_ID_PID_TUNE_LEN, MAVLINK_MSG_ID_PID_TUNE_CRC);
#else
    mavlink_pid_tune_t packet;
    packet.time_boot_ms = time_boot_ms;
    packet.Cmd_In = Cmd_In;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PID_TUNE, (const char *)&packet, MAVLINK_MSG_ID_PID_TUNE_MIN_LEN, MAVLINK_MSG_ID_PID_TUNE_LEN, MAVLINK_MSG_ID_PID_TUNE_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_pid_tune_send_struct(mavlink_channel_t chan, const mavlink_pid_tune_t* pid_tune)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_pid_tune_send(chan, pid_tune->time_boot_ms, pid_tune->Cmd_In, pid_tune->Sensor_Fbk, pid_tune->Cmd_Out, pid_tune->Kp, pid_tune->Ki, pid_tune->Kd);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_PID_TUNE, (const char *)pid_tune, MAVLINK_MSG_ID_PID_TUNE_MIN_LEN, MAVLINK_MSG_ID_PID_TUNE_LEN, MAVLINK_MSG_ID_PID_TUNE_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_PID_TUNE_LEN? msg->len : MAVLINK_MSG_ID_PID_TUNE_LEN;
        memset(pid_tune, 0, MAVLINK_MSG_ID_PID_TUNE_LEN);
    memcpy(pid_tune, _MAV_PAYLOAD(msg), len);
#endif
}
