...
#define MAVLINK_MSG_ID_Lateral_MIN_LEN 52
#define MAVLINK_MSG_ID_192_LEN 52
#define MAVLINK_MSG_ID_192_MIN_LEN 52
                               ...
                               uint32_t time_boot_ms, float Beta, float p, float r, float ay, float IMU_roll, float IMU_yaw, float GBL_roll, float GBL_yaw, float Rudder, float Aileron, float Rudder_cmd, float Aileron_cmd)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_Lateral_MIN_LEN, MAVLINK_MSG_ID_Lateral_LEN, MAVLINK_MSG_ID_Lateral_CRC);
}
                                   ...
                                   uint32_t time_boot_ms,float Beta,float p,float r,float ay,float IMU_roll,float IMU_yaw,float GBL_roll,float GBL_yaw,float Rudder,float Aileron,float Rudder_cmd,float Aileron_cmd)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_Lateral;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_Lateral_MIN_LEN, MAVLINK_MSG_ID_Lateral_LEN, MAVLINK_MSG_ID_Lateral_CRC);
}
...
static inline void mavlink_msg_lateral_send(mavlink_channel_t chan, uint32_t time_boot_ms, float Beta, float p, float r, float ay, float IMU_roll, float IMU_yaw, float GBL_roll, float GBL_yaw, float Rudder, float Aileron, float Rudder_cmd, float Aileron_cmd)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_Lateral, buf, MAVLINK_MSG_ID_Lateral_MIN_LEN, MAVLINK_MSG_ID_Lateral_LEN, MAVLINK_MSG_ID_Lateral_CRC);
#else
    mavlink_lateral_t packet;
    packet.time_boot_ms = time_boot_ms;
    packet.Beta = Beta;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_Lateral, (const char *)&packet, MAVLINK_MSG_ID_Lateral_MIN_LEN, MAVLINK_MSG_ID_Lateral_LEN, MAVLINK_MSG_ID_Lateral_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_lateral_send_struct(mavlink_channel_t chan, const mavlink_lateral_t* lateral)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_lateral_send(chan, lateral->time_boot_ms, lateral->Beta, lateral->p, lateral->r, lateral->ay, lateral->IMU_roll, lateral->IMU_yaw, lateral->GBL_roll, lateral->GBL_yaw, lateral->Rudder, lateral->Aileron, lateral->Rudder_cmd, lateral->Aileron_cmd);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_Lateral, (const char *)lateral, MAVLINK_MSG_ID_Lateral_MIN_LEN, MAVLINK_MSG_ID_Lateral_LEN, MAVLINK_MSG_ID_Lateral_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_Lateral_LEN? msg->len : MAVLINK_MSG_ID_Lateral_LEN;
        memset(lateral, 0, MAVLINK_MSG_ID_Lateral_LEN);
    memcpy(lateral, _MAV_PAYLOAD(msg), len);
#endif
}
